...
fn solve(io: &mut Io<Reader, Stdout>) {
  let n: i64 = io.next();
  let mut rems: HashMap<i64, u64> = HashMap::from([(0, 1)]);
  let mut current_rem: i64 = 0;
  let mut count: u64 = 0;
  for _ in 0..n {
    current_rem = ((current_rem + io.next::<i64>()) % n + n) % n ;
    if let Some(found) = rems.get(&current_rem) {
      count += found;
    }
    *rems.entry(current_rem).or_default() += 1;
  }
  io.writeln(count);
}
