...
fn solve(io: &mut Io<Reader, Stdout>) {
  let n_tickets: usize = io.next();
  let n_customers: usize = io.next();
  let mut tickets: BTreeMap<u32, u32> = BTreeMap::new();
  for _ in 0..n_tickets {
    let key = io.next();
    tickets.entry(key).and_modify(|v| *v += 1).or_insert(1);
  }
  for _ in 0..n_customers {
    let customer_price: u32 = io.next();

    match tickets.range(..=customer_price).next_back() {
      Some((&price, &count)) => {
        io.writeln(price);
        if count > 1 {
          *tickets.get_mut(&price).unwrap() -= 1;
        } else {
