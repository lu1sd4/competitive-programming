...
fn solve_game(source: u8, target: u8, spare: u8, n: u16, io: &mut Io<Reader, Stdout>) {
  if n == 0 {
    return;
  }
  solve_game(source, spare, target, n - 1, io);
  io.writeln(format!("{} {}", &source, &target));
  solve_game(spare, target, source, n - 1, io);
}

fn solve(io: &mut Io<Reader, Stdout>) {
  let n: u16 = io.next();
  io.writeln((1 << n) - 1);
  solve_game(1, 3, 2, n, io);
}
