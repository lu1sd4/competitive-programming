...
fn solve(io: &mut Io<Reader, Stdout>) {
  let n: usize = io.next();
  let target: u32 = io.next();
  let mut numbers: Vec<(u32, usize)> = (0..n).map(|i| (io.next(), i + 1)).collect();
  numbers.sort_by_key(|e| e.0);
  let mut left = 0;
  let mut right = numbers.len() - 1;
  while left < right {
    let looking_for = target - numbers[left].0;
    ...
    if n_right > looking_for {
      ...
      return io.writeln(format!("{} {}", numbers[left].1, numbers[right].1));
    }
  }
  io.writeln("IMPOSSIBLE");
}

fn solve_map(io: &mut Io<Reader, Stdout>) {
  let n: usize = io.next();
  let target: i32 = io.next();
  let mut seen: HashMap<i32, usize> = HashMap::new();
  for i in 1..=n {
    let a: i32 = io.next();
    if let Some(j) = seen.get(&(target - a)) {
      return io.writeln(format!("{} {}", i, j));
    }
    seen.insert(a, i);
  }
  io.writeln("IMPOSSIBLE");
