...
fn solve(io: &mut Io<Reader, Stdout>) {
  let n: usize = io.next();
  let mut peaks: BTreeMap<u32, u32> = BTreeMap::new();
  let mut piles = 0;
  for _ in 0..n {
    let k: u32 = io.next();
    match peaks.range((Excluded(k), Unbounded)).next() {
      Some((&higher_peak, &count)) => {
        if count > 1 {
          *peaks.get_mut(&higher_peak).unwrap() -= 1;
        } else {
          peaks.remove(&higher_peak);
        }
      }
      None => {
        piles += 1;
    ...
    peaks.entry(k).and_modify(|count| *count += 1).or_insert(1);
  }
  io.writeln(piles);
}
