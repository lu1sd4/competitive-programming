#![allow(unused)]
use std::cmp::{max, min, Reverse};
use std::collections::{BTreeMap, BTreeSet, BinaryHeap, HashMap, HashSet, VecDeque};
use std::fs::File;
use std::io::{self, BufRead, BufReader, BufWriter, Read, Stdout, Write};
use std::str::FromStr;

fn main() {
  let reader = open_input();
  let writer = io::stdout();
  let mut io = Io::new(reader, writer);

  solve_short(&mut io);
  io.flush();
}

type Reader = Box<dyn Read>;

fn solve_short(io: &mut Io<Reader, Stdout>) {
  let n: u64 = io.next();
  let mut last_seen: HashMap<u64, u64> = HashMap::new();
  let mut left: u64 = 0;
  let mut total: u64 = 0;
  for i in 0..n {
    let x = io.next();
    if let Some(previous) = last_seen.insert(x, i) {
      left = left.max(previous + 1);
    }
    total += i - left + 1;
  }
  io.writeln(total);
}

fn count_subarrays(n: u64) -> u64 {
  (n * (n + 1)) / 2
}

fn solve(io: &mut Io<Reader, Stdout>) {
  let n: u64 = io.next();
  let mut last_seen: HashMap<u64, u64> = HashMap::new();
  let mut total: u64 = 0;
  let mut left: u64 = 0;
  let mut right: u64 = 0;
  let mut previous_right: u64 = 0;
  for i in 0..n {
    let x = io.next();
    if let Some(previous_position) = last_seen.insert(x, i) {
      if previous_position >= left {
        let right = i;
        let current_length = right - left;
        let already_counted_length = previous_right - left;
        let to_add = count_subarrays(current_length) - count_subarrays(already_counted_length);
        total += to_add;
        left = previous_position + 1;
        previous_right = right;
      }
    }
  }
  total += count_subarrays(n - left) - count_subarrays(previous_right - left);
  io.writeln(total);
}

fn open_input() -> Reader {
  match std::env::args().nth(1) {
    Some(path) => {
      let f = File::open(&path).unwrap_or_else(|_| panic!("could not open input file: {}", path));
      Box::new(f)
    }
    None => Box::new(io::stdin()),
  }
}

struct Io<R: Read, W: Write> {
  input: BufReader<R>,
  output: BufWriter<W>,
}

impl<R: Read, W: Write> Io<R, W> {
  fn new(reader: R, writer: W) -> Self {
    Io {
      input: BufReader::new(reader),
      output: BufWriter::new(writer),
    }
  }

  fn next<T: FromStr>(&mut self) -> T {
    let token: Vec<u8> = self
      .input
      .by_ref()
      .bytes()
      .map(|b| b.expect("failed to read a byte from input"))
      .skip_while(|b| b.is_ascii_whitespace())
      .take_while(|b| !b.is_ascii_whitespace())
      .collect();
    std::str::from_utf8(&token)
      .unwrap()
      .parse()
      .ok()
      .unwrap_or_else(|| panic!("could not parse token as requested type"))
  }

  fn next_vec<T: FromStr>(&mut self, n: usize) -> Vec<T> {
    (0..n).map(|_| self.next()).collect()
  }

  fn next_line(&mut self) -> String {
    let mut s = String::new();
    self
      .input
      .read_line(&mut s)
      .expect("failed to read a line from input");
    s.trim_end().to_string()
  }

  fn next_grid(&mut self, n: usize) -> Vec<Vec<u8>> {
    (0..n).map(|_| self.next_line().into_bytes()).collect()
  }

  fn next_char(&mut self) -> Option<char> {
    self
      .input
      .by_ref()
      .bytes()
      .map(|b| b.expect("failed to read a byte from input"))
      .find(|b| !b.is_ascii_whitespace())
      .map(|b| b as char)
  }

  fn write<T: std::fmt::Display>(&mut self, val: T) {
    write!(self.output, "{}", val).unwrap();
  }

  fn write_sp<T: std::fmt::Display>(&mut self, val: T) {
    write!(self.output, "{} ", val).unwrap();
  }

  fn writeln<T: std::fmt::Display>(&mut self, val: T) {
    writeln!(self.output, "{}", val).unwrap();
  }

  fn flush(&mut self) {
    self.output.flush().unwrap();
  }
}

impl<R: Read, W: Write> Drop for Io<R, W> {
  fn drop(&mut self) {
    let _ = self.output.flush();
  }
}
